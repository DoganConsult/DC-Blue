...
@Component({
  ...
  selector: 'app-thanks',
  template: `
    <div class="min-h-screen bg-gradient-to-br from-[var(--brand-dark)] via-[#0c4a82] to-[var(--brand-darker)] flex items-center justify-center px-4">
      <div class="max-w-lg w-full text-center">
        ...
        @if (ticket()) {
          <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 mb-8">
            ...
            <p class="text-3xl font-mono font-bold text-[var(--gold)] tracking-wider">{{ ticket() }}</p>
        ...
        <!-- What's Next -->
        <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 mb-8 text-left">
          <h3 class="text-white font-semibold mb-4">{{ i18n.t("What's Next?", 'ماذا بعد؟') }}</h3>
          <div class="space-y-3">
            <div class="flex items-start gap-3">
              <span class="flex-shrink-0 w-7 h-7 rounded-full bg-[var(--primary)]/20 text-[var(--primary)] flex items-center justify-center text-sm font-bold">1</span>
              <p class="text-white/70 text-sm">{{ i18n.t('Our team reviews your requirements', 'فريقنا يراجع متطلباتك') }}</p>
            </div>
            <div class="flex items-start gap-3">
              <span class="flex-shrink-0 w-7 h-7 rounded-full bg-[var(--primary)]/20 text-[var(--primary)] flex items-center justify-center text-sm font-bold">2</span>
              <p class="text-white/70 text-sm">{{ i18n.t('We schedule a discovery call', 'نُجدول مكالمة استكشافية') }}</p>
            </div>
            <div class="flex items-start gap-3">
              <span class="flex-shrink-0 w-7 h-7 rounded-full bg-[var(--primary)]/20 text-[var(--primary)] flex items-center justify-center text-sm font-bold">3</span>
        ...
        <div class="flex flex-col sm:flex-row gap-3 justify-center">
          <button (click)="goHome()"
                  class="px-8 py-3 rounded-xl font-semibold bg-gradient-to-r from-[var(--primary)] to-[#2563EB] text-white
          ...
          <button (click)="newInquiry()"
                  class="px-8 py-3 rounded-xl font-semibold border border-white/20 text-white hover:bg-white/10 transition-all">
