  ...
  template: `
    <div class="min-h-screen bg-gradient-to-br from-[var(--brand-dark)] via-[#0c4a82] to-[var(--brand-darker)]">
      ...
      <nav class="flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
        <a routerLink="/" class="text-white font-bold text-xl tracking-tight cursor-pointer" (click)="goHome()">
      ...
      <div class="max-w-3xl mx-auto px-4 py-12">
        ...
        <!-- Form Card -->
        <form #f="ngForm" (ngSubmit)="submit()" class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-10 space-y-6">
          ...
          <div>
            ...
            <select name="product_line" [(ngModel)]="form.product_line" required
                    class="w-full bg-white/10 text-white border border-white/20 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--primary)]">
              <option value="" disabled class="text-gray-900">{{ i18n.t('Select...', 'اختر...') }}</option>
          ...
          <div>
            ...
            <select name="vertical" [(ngModel)]="form.vertical"
              ...
              <option value="government" class="text-gray-900">{{ i18n.t('Government', 'الحكومة') }}</option>
              <option value="banking" class="text-gray-900">{{ i18n.t('Banking & Finance', 'البنوك والمالية') }}</option>
              <option value="healthcare" class="text-gray-900">{{ i18n.t('Healthcare', 'الرعاية الصحية') }}</option>
              <option value="energy" class="text-gray-900">{{ i18n.t('Energy & Utilities', 'الطاقة والمرافق') }}</option>
              <option value="telecom" class="text-gray-900">{{ i18n.t('Telecom', 'الاتصالات') }}</option>
              <option value="retail" class="text-gray-900">{{ i18n.t('Retail & E-Commerce', 'التجزئة والتجارة الإلكترونية') }}</option>
              <option value="education" class="text-gray-900">{{ i18n.t('Education', 'التعليم') }}</option>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input name="company_name" [(ngModel)]="form.company_name" required
                     ...
                     class="w-full bg-white/10 text-white placeholder-white/40 border border-white/20 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--primary)]" />
            </div>
            <!-- CR Number -->
            <div>
            ...
            <div>
              ...
              <select name="company_size" [(ngModel)]="form.company_size"
                ...
                @for (o of companySizeOptions; track o.value) {
                  <option [value]="o.value" class="text-gray-900">{{ i18n.lang() === 'ar' ? o.labelAr : o.labelEn }}</option>
                }
              </select>
            </div>
            <div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <select name="expected_users" [(ngModel)]="form.expected_users"
                      class="w-full bg-white/10 text-white border border-white/20 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--primary)]">
                <option value="" class="text-gray-900">{{ i18n.t('Select...', 'اختر...') }}</option>
                <option value="1-50" class="text-gray-900">1 – 50</option>
                <option value="51-200" class="text-gray-900">51 – 200</option>
                <option value="201-500" class="text-gray-900">201 – 500</option>
                <option value="501-1000" class="text-gray-900">501 – 1,000</option>
                <option value="1000+" class="text-gray-900">1,000+</option>
              </select>
            </div>
          ...
          <div>
            ...
            <select name="timeline" [(ngModel)]="form.timeline"
                    class="w-full bg-white/10 text-white border border-white/20 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--primary)]">
              <option value="" class="text-gray-900">{{ i18n.t('Select...', 'اختر...') }}</option>
              <option value="immediate" class="text-gray-900">{{ i18n.t('Immediate (< 1 month)', 'فوري (أقل من شهر)') }}</option>
              <option value="1-3months" class="text-gray-900">1 – 3 {{ i18n.t('months', 'أشهر') }}</option>
              <option value="3-6months" class="text-gray-900">3 – 6 {{ i18n.t('months', 'أشهر') }}</option>
              <option value="6months+" class="text-gray-900">6+ {{ i18n.t('months', 'أشهر') }}</option>
            </select>
          </div>
          ...
          <div>
            ...
            <textarea name="message" [(ngModel)]="form.message" rows="4"
                      ...
                      class="w-full bg-white/10 text-white placeholder-white/40 border border-white/20 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] resize-none"></textarea>
          </div>
          ...
          <label class="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" name="consent_pdpl" [(ngModel)]="form.consent_pdpl"
                   class="mt-1 w-5 h-5 rounded border-white/20 bg-white/10 text-[var(--primary)] focus:ring-[var(--primary)]" />
          ...
          <button type="submit" [disabled]="loading() || !f.valid || !form.consent_pdpl"
                  class="w-full py-4 rounded-xl font-semibold text-lg transition-all duration-200
                         bg-gradient-to-r from-[var(--primary)] to-[#2563EB] text-white
